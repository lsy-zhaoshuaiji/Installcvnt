...
def install_cvnt(docker_num,cvnt_uid,mount_dev):
    ...
    for i in range(docker_num):
        ...
        run_container = "docker run -d -it --name {}{} --restart=always" \
                        ...
                        "-h {} -m 2G " \
                        "registry.cn-hangzhou.aliyuncs.com/cs_work/cvnt_work:v6.0 ".format(container_name,dokcer_mount, mac, port_web,
                                                                                          a, b, c, container_name)
        # print(run_container)
        container_Id = subprocess.getoutput(run_container)
        start_cvnt = r'docker exec %s bash -c "cd /usr/local/yyets_20190918/;cd conf;' \
                     r"echo -e '#本地账户信息\nuid       = %d\n# 登录后的token-系统生成\nsys_token =' > " \
                     r'user.conf;'\
                     r"echo -e 'nameserver 8.8.8.8\nnameserver 8.8.4.4' > /etc/resolv.conf;"\
                     r'cd /usr/local/yyets_20190918/;./install.sh start;"' % (container_Id,cvnt_uid)
        # start_cvnt = r'docker exec %s bash -c "cd /usr/local/yyets_20190829/yyets_20190829/;cd conf;' \
        #                 r"echo -e '#本地账户信息\nuid= %d' > " \
        #                  r'user.conf;' \
        #                  r"echo -e 'nameserver 8.8.8.8\nnameserver 8.8.4.4' > /etc/resolv.conf;" \
        #                  r'cd /usr/local/yyets_20190829/yyets_20190829;./install.sh start;"' % (container_Id,cvnt_uid)
